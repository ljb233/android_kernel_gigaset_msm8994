...
static int tfa98xx_wait_calibration(struct tfa98xx *tfa98xx, int *done)
{
	...
	*done = 0;
	...
	if (mtp & TFA98XX_MTP_MTPOTC) {
		...
		pr_info("poll xmem for calibrate always\n");
		while ((*done == 0) && (tries < TFA98XX_WAITRESULT_NTRIES)) {
			msleep_interruptible(5);
...
static int tfa98xx_set_mute(struct tfa98xx *tfa98xx, enum Tfa98xx_Mute mute)
{
	...
	u16 audioctrl_value;
	u16 sysctrl_value;
	int audioctrl_value_32;
	int sysctrl_value_32;

	pr_debug("\n");

	audioctrl_value_32 = snd_soc_read(codec, TFA98XX_AUDIO_CTR);
	sysctrl_value_32 = snd_soc_read(codec, TFA98XX_SYS_CTRL);
	
	dev_info(codec->dev, "audioctrl_value = 0x%08x, sysctrl_value=0x%08x\n", audioctrl_value_32, sysctrl_value_32);
	if(audioctrl_value_32 < 0)
		audioctrl_value = 0x000f;
	else 
		audioctrl_value = (u16)audioctrl_value_32;
	if(sysctrl_value_32 < 0)
		sysctrl_value = 0x827c;
	else
		sysctrl_value = (u16)sysctrl_value_32;
...
int tfa98xx_dsp_get_calibration_impedance(struct tfa98xx *tfa98xx, u32 *re25)
{
	...
	*re25 = (TO_FIXED(data[0]) / (1 << (23 - SPKRBST_TEMPERATURE_EXP))) * 100;
...
static int tfa98xx_aec_output(struct tfa98xx *tfa98xx, int enable)
{
	...
	value = snd_soc_read(codec, TFA98XX_I2SREG);
	pr_debug("%s:0x4 0x%x", __func__, value);

	value = snd_soc_read(codec, TFA98XX_I2S_SEL_REG);
	if (enable == 0){
		value &= ~(0x7 << 0);
		value |= (0x3 << 0);
		value &= ~(0x7 << 3);
	...
	ret = snd_soc_write(codec, TFA98XX_I2S_SEL_REG, value);
	value = snd_soc_read(codec, TFA98XX_I2S_SEL_REG);

	pr_debug("%s:0xa 0x%x", __func__, value);
	return ret;
}
...
#if SMARTPA_FCT_TEST
#define PARAM_GET_LSMODEL           0x86        // Gets current LoudSpeaker impedance Model.阻抗模型 f0 会
